...
Aggregate command behavior and typed-result behavior are version 99 after
rejecting unescaped TeX special characters that require literal escapes. Asset
...
version 3, and Structured content is version 86. The immediately previous
aggregate version 98 rejects instead of being reinterpreted.
...
Full-profile validation now also produces constructor-sealed reusable evidence
that retains the exact admitted profile, oriented sheet, and writable rectangle.
The same 20,000 valid and 120,000 mixed-profile corpora require this sealed view
to preserve exact geometry and first-error precedence; it adds no layout policy.
Measured-flow pagination, fixed-region bounds, and profile paper-mark
compilation now reuse that sealed geometry instead of separately re-deriving
validated facts.
...
special characters, with unescaped `%`, `#`, and `$` rejected as typed syntax
...
environments remain explicit unsupported presentation semantics rather than
being accepted as literal formula content. Matrix and small-matrix leading
brackets remain ordinary content because those admitted environments own no such
optional argument.
...
missing, invalid, or out-of-range positions.

Internal provider indexes cannot alias byte zero or the final source byte, and
each internal index must leave enough UTF-8 bytes on both sides for the
advertised prefix and suffix grapheme counts. An internal cursor or insertion
therefore cannot silently jump to an impossible source position. All 114 frozen
...
allowing the mode-specific treatments themselves to differ. Successful
comparison now returns constructor-sealed evidence borrowing the exact plan.

A 256-case compact oracle crosses 64 hierarchy values with both identity- and
hierarchy-drift bits while varying digital/live treatment values independently
and requires direct/sealed admission parity.
