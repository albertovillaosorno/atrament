...
An application-level repeated-No-op control now detects that evidence from two
consecutive qualified semantic-command results only when both are No-op for the
same owner-qualified bounded intent and no new relevant evidence was supplied.
The same boundary exposes the qualified condition as a local requirement to stop
equivalent mutation attempts. It does not infer intent identity or evidence
relevance, manufacture a terminal goal reason or successful completion, or grant
permission to continue when this specific stop axis is absent.
...
The same boundary also consumes read-only history availability before mutation,
preserving requested-direction states as admitted, boundary, or no accepted
revision instead of probing Undo/Redo merely to discover availability.
...
blocking-evidence, and bounded-intent fingerprints plus an explicit fact stating
