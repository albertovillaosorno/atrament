...
typed-result behavior are version 87 after completing exact-source AMS ordinary
symbol declarations with `\backprime` and `\complement`.
Asset reference, Ordering and grouping, and Text content retain family behavior
version 1; Document constraint is version 3;
Structured content is version 75; Provenance and Style role are version 2.
