...
typed-result behavior are version 96 after admitting the remaining `latexsym`
join and triangle atoms. Asset reference, Ordering and grouping, and Text
content retain family behavior version 1; Document constraint is version 3;
Structured content is version 83; Provenance and Style role are version 2.
...
and command-count admission remain independent at this boundary. Invalid graph
shape is intentionally not repaired here: a fixture preserves duplicate command
identities and a missing dependency for the separate graph validator while
preflight reports only the facts it owns.
