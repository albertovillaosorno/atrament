...
#[test]
fn aligned_environment_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = concat!(
        r"\begin{aligned}y &= x^2 \\ ",
        r"y' &= 2x\end{aligned}",
    );
    let (candidate, formula) =
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("aligned environment mathematics must be accepted");
    };
    ...
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted aligned environment mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("aligned environment mathematics edit must apply: {outcome:?}");
    };
...
#[test]
fn braced_matrix_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"A=\begin{Bmatrix}1 & 2 \\ 3 & 4\end{Bmatrix}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("brace-delimited matrix mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted brace-delimited matrix mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let edited_source = r"A=\begin{Bmatrix}a & b \\ c & d\end{Bmatrix}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Display,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("brace-delimited matrix edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("edited brace-delimited matrix mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn bracketed_matrix_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"A=\begin{bmatrix}1 & 2 \\ 3 & 4\end{bmatrix}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("bracketed matrix mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted bracketed matrix mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let edited_source = r"B=\begin{bmatrix}a & b \\ c & d\end{bmatrix}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Display,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("bracketed matrix edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited bracketed matrix mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn double_vertical_matrix_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"D=\begin{Vmatrix}1 & 2 \\ 3 & 4\end{Vmatrix}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("double-vertical-bar matrix mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted double-vertical-bar matrix mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let edited_source = r"D=\begin{Vmatrix}a & b \\ c & d\end{Vmatrix}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Display,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("double-vertical-bar matrix edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("edited double-vertical-bar matrix mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn vertical_matrix_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"D=\begin{vmatrix}1 & 2 \\ 3 & 4\end{vmatrix}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("vertical-bar matrix mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted vertical-bar matrix mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("vertical-bar matrix edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("edited vertical-bar matrix mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn small_matrix_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"A=\begin{smallmatrix}1 & 2 \\ 3 & 4\end{smallmatrix}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Inline);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("compact small-matrix mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("accepted compact small-matrix mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let edited_source = r"A=\begin{smallmatrix}a & b \\ c & d\end{smallmatrix}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("compact small-matrix edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session
                .current()
                .expect("edited compact small-matrix mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn split_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = concat!(
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("split mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted split mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("split mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited split mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn gathered_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = concat!(
        r"\begin{gathered}x+y \\ ",
        r"z-w\end{gathered}",
    );
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("gathered mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted gathered mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("gathered mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited gathered mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn substack_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\sum_{\substack{i=1\\j=1}} a_{ij}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Inline);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("substack mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted substack mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("substack mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited substack mathematics"),
            formula,
        )
        .source,
        edited_source,
...
fn expanded_tex_vocabulary_is_admitted_and_directly_editable() {
    ...
    let edited_source = concat!(
        r"\iint_D f dA + \bigcup_i A_i + \aleph_0 + \wp(A); ",
        ...
        r"\varbigtriangleup A; \mathsection + \mathdollar; ",
        r"\varGamma + \varOmega; \injlim_i A_i + \varprojlim_j B_j; ",
        r"P \impliedby Q; \iiiint_A f; \lvert x \rvert; y \pod{23}; ",
        r"\beth + \gimel + \varnothing + \hslash; ",
        r"\square + \blacklozenge + \measuredangle ABC; ",
        r"A \boxplus B \curlywedge C \smallsetminus D; ",
        r"a \leqq b \gtrapprox c \triangleq d; ",
        r"x \twoheadrightarrow y \rightsquigarrow z \multimap w; ",
        r"A \Subset B \trianglelefteq C \blacktriangleright D; ",
        r"P \Vdash Q \shortparallel R \pitchfork S; ",
        r"U \nrightarrow V \nLeftrightarrow W; ",
        r"a \nleqq b \precnapprox c \ncong d; ",
        r"A \nsubseteqq B \varsubsetneqq C; ",
        r"P \nparallel Q \nVdash R \ntrianglelefteq S; ",
        r"\backprime + \complement",
    );
    let outcome = session.replace_formula(
        revision,
        formula,
...
#[test]
fn stacked_relation_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"a\stackrel{!}{=}b";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Inline);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("stacked relation mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted stacked relation"),
            formula,
        )
        .source,
        initial,
    );
    let edited_source = r"x\stackrel{def}{=}y";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("stacked relation edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited stacked relation"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn stacked_annotation_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"x \overset{!}{=} y";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("stacked-annotation mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted stacked-annotation mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("stacked-annotation mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited stacked-annotation mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn boxed_and_brace_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\boxed{x+1} + \overbrace{a+b}^{sum}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("boxed and brace mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted boxed and brace mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("boxed and brace mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited boxed and brace mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn directional_over_arrow_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\overrightarrow{AB}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Inline);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("directional over-arrow mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted directional over-arrow"),
            formula,
        )
        .source,
        initial,
    ...
    let edited_source = r"\underleftrightarrow{CD}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("directional over-arrow edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited directional over-arrow"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn styled_binomial_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\dbinom{n}{k}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("styled binomial mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted styled binomial"),
            formula,
        )
        .source,
        initial,
    );
    let edited_source = r"\tbinom{a}{b}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("styled binomial edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    let edited_formula = formula_value_for_test(
        session.current().expect("edited styled binomial"),
        formula,
    );
    assert_eq!(edited_formula.mode, FormulaMode::Inline);
    assert_eq!(edited_formula.source, edited_source);
}
...
#[test]
fn arrow_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"A \Leftarrow B \leftrightarrow C \Longrightarrow D";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("arrow mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted arrow mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("arrow mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited arrow mathematics"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn named_delimiter_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\langle x, y \rangle + \lceil z \rceil";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Display);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("named-delimiter mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted named-delimiter mathematics"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("named-delimiter mathematics edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited named-delimiter mathematics"),
            formula,
        )
        .source,
        edited_source,
...
fn extended_accent_tex_is_admitted_and_directly_editable() {
    ...
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("extended-accent mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted extended accent"),
            formula,
        )
        .source,
        initial,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("extended-accent edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited extended accent"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn wide_accent_tex_is_admitted_and_directly_editable() {
    let ids = IdentityAllocator::new();
    let initial = r"\widehat{ABC}";
    let (candidate, formula) =
        candidate_math_notebook(&ids, initial, FormulaMode::Inline);
    let mut session = SemanticNotebookSessionService::default();
    let AcceptanceOutcome::Accepted { mapping, revision } =
        ...
        panic!("wide-accent mathematics must be accepted");
    };
    let formula = accepted_for(&mapping, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("accepted wide accent"),
            formula,
        )
        .source,
        initial,
    );
    let edited_source = r"\widetilde{xyz}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
    ...
    let FormulaEditOutcome::Applied {
        ...
        panic!("wide-accent edit must apply: {outcome:?}");
    };
    assert_eq!(base, revision);
    assert_ne!(edited, revision);
    assert_eq!(target, formula);
    assert_eq!(
        formula_value_for_test(
            session.current().expect("edited wide accent"),
            formula,
        )
        .source,
        edited_source,
...
#[test]
fn environment_closing_inside_group_rejects_candidate_atomically() {
    let ids = IdentityAllocator::new();
    let valid = candidate_notebook(&ids, "accepted text");
...
fn command_capability_snapshot_is_deterministic_and_does_not_overclaim() {
    ...
    assert_eq!(snapshot.family_capabilities, [
        ...
        CommandFamilyCapability {
            behavior_version: CommandBehaviorVersion(75),
...
fn command_capability_version_detects_drift_independently_of_revision() {
    ...
    assert_eq!(
        session.check_command_capability_compatibility(
            CommandBehaviorVersion(86),
        ),
        CommandCapabilityCompatibilityOutcome::Mismatch {
            current: CURRENT_COMMAND_BEHAVIOR_VERSION,
            expected: CommandBehaviorVersion(86),
        },
    );
    assert_eq!(
        session
