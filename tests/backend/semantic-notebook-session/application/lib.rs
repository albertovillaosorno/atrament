...
fn expanded_tex_vocabulary_is_admitted_and_directly_editable() {
    ...
    let edited_source = concat!(
        ...
        r"\bigsqcup_i S_i; A \sqsubseteq B; x \odot y; p \bowtie q; a \| b",
    );
    let outcome = session.replace_formula(
        revision,
        formula,
...
fn directional_over_arrow_tex_is_admitted_and_directly_editable() {
    ...
    let edited_source = r"\overleftrightarrow{CD}";
    let outcome = session.replace_formula(
        revision,
        formula,
        FormulaMode::Inline,
...
fn command_capability_snapshot_is_deterministic_and_does_not_overclaim() {
    ...
    assert_eq!(snapshot.family_capabilities, [
        ...
        CommandFamilyCapability {
            behavior_version: CommandBehaviorVersion(52),
...
fn command_capability_version_detects_drift_independently_of_revision() {
    ...
    assert_eq!(
        session.check_command_capability_compatibility(
            CommandBehaviorVersion(63),
        ),
        CommandCapabilityCompatibilityOutcome::Mismatch {
            current: CURRENT_COMMAND_BEHAVIOR_VERSION,
            expected: CommandBehaviorVersion(63),
        },
    );
    assert_eq!(
        session
