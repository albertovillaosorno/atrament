...
fn page_profile_edits_recompute_live_fixed_region_geometry() {
    ...
    let edited_bound =
        RevisionLayoutDiagnostics::bind(edited, &edited_diagnostics)
            .expect("edited overflow diagnostics must bind");
    assert!(matches!(
    ...
    let redone_bound =
        RevisionLayoutDiagnostics::bind(redone, &redone_diagnostics)
            .expect("Redone overflow diagnostics must bind freshly");
    assert!(matches!(
...
fn cursor_step_and_selection_queries_follow_history_revision_freshness() {
    ...
    assert_eq!(
        ...
        Ok(application::TextGraphemeSelectionOutcome::StaleBase {
            current: edited,
        }),
    );
    assert_eq!(
...
struct ShiftedFirstAdvertisedBoundary;

impl GraphemeBoundaryProvider for ShiftedFirstAdvertisedBoundary {
    fn byte_offset(
        &self,
        source: &str,
        grapheme_index: usize,
